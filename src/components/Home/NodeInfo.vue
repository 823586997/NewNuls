    ...
    <div class="nodeInfo">
        <div class='nodeInfo_cen' v-for='(item , index) in data' :key='index'>
            <div class='nickname'>
                <p>
                    <span class='avatar'>
                        <img :src=" item.avatar ? item.avatar : require('../../static/images/default.jpg') " alt="">
                    </span>
                </p>
                <span>
                    <i v-if=" ( role == 'admin' || nodeId == item.id ) && item.checkstatus == '0' " class='buldGreen'></i>
                    <i v-if=" ( role == 'admin' || nodeId == item.id ) && item.checkstatus == '2' " class='buldRed'></i>
                    <i v-if=" ( role == 'admin' || nodeId == item.id ) && item.checkstatus == '1' " class='buldYellow'></i>
                    <i class='message' @click='Details(item.id)'></i>
                    <i v-if="role == 'admin' || nodeId == item.id" class='redact' @click='Change(item.id)'></i>
                </span>
            </div>
            <div class='message'>
                <div>
